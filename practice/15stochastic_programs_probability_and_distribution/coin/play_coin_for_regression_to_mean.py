...
"""
这里不探究当次数接近于无穷时,投掷到正面（也是是head）的概率是不是1/2，这个可以参考roulette 和 play_coin_for_law_of_large_numbers。
这里探究均值回归问题。

均值回归描述的是 ，如果极端事件发生后，下一次的事件将不那么极端,将更加接近事件的均值。比如连续5次出现head，那么下面连续5次
中出现head的次数可能小于5次,更加接近出现2.5次head。但这也是一种可能性，并不是说这种极端事件就绝不再次发生。
