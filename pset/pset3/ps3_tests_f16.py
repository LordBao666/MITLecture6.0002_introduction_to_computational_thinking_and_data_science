...
"""
ImportError: bad magic number in 'test': b'\x16\r\r\n'
.pyc 貌似是 执行.py会产生的一种中间结果,这个不重要,重要的是不同版本解释器针对同一个
.py文件会产生不同的.pyc。

也就是发生上述ImportError的原因可能是python版本对不上号。
我的是3.8。如果是PyCharm,.pyc文件可以在.py文件的同级文件夹_pycache_中可以看见。
但是这个test.pyc 的解释器 是 3.5 （MIT6.0002用的就是3.5)

test = imp.load_compiled("test", "test.pyc")

我自己用Anaconda 下载了 python3.5版本,但是PyCharm不支持这个3.5版本。
所以我看了一下测试代码,自己去修改了一下。

简单来说 就是 将test  都修改为  ps3
比如 test.Position()  修改为 ps3.Position()
