...
class RectangularRoom(object):
    ...
    #     """
    #     Rectangular的实例是不允许存在的
    #     """
    #     if self.__class__.__name__ == RectangularRoom.__name__:
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        unit_x = math.floor(pos.get_x())
        unit_y = math.floor(pos.get_y())
        if (unit_x, unit_y) not in self.room:
            raise ValueError("Invalid Pos")
        else:
            if self.room[(unit_x, unit_y)] <= capacity:
                self.room[(unit_x, unit_y)] = 0
            else:
                self.room[(unit_x, unit_y)] -= capacity
    ...
    def is_position_in_room(self, pos):
        ...
        unit_x = math.floor(pos.get_x())
        unit_y = math.floor(pos.get_y())
        return (unit_x, unit_y) in self.room
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        random_x = random.random() * room_width
        random_y = random.random() * room_height

        self.position = Position(random_x, random_y)
        self.direction = random.random() * 360  # 随机产生 [0,360)的角度
        self.speed = speed
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_furnished(self, pos):
        ...
        unit_x = math.floor(pos.get_x())
        unit_y = math.floor(pos.get_y())
        return (unit_x, unit_y) in self.furniture_tiles
    ...
    def is_position_valid(self, pos):
        ...
        """
        return (0 <= pos.get_x() <= self.width and 0 <= pos.get_y() <= self.height) and \
               (math.floor(pos.get_x()), math.floor(pos.get_y())) not in self.furniture_tiles
    ...
    def get_random_position(self):
        ...
        random_x = random.random() * self.width
        random_y = random.random() * self.height
        unit_x = math.floor(random_x)
        unit_y = math.floor(random_y)
        while (unit_x, unit_y) in self.furniture_tiles:
            random_x = random.random() * self.width
            random_y = random.random() * self.height
            unit_x = math.floor(random_x)
            unit_y = math.floor(random_y)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    """
    """
    根据题意 in a room of dimensions width x height
    with the dirt dirt_amount on each tile.这种Room 应该是 EmptyRoom
...
#       StandardRobot is much better than FaultyRobot。 20 * 15  用时最短
#
